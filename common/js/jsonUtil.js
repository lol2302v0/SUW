...
function jsonGetVal(parent, path) {
	if (path.length == 1)
		return (isUndef(parent) || isNull(parent)) ? null : ((isUndef(parent[path[0]]) || isNull(parent[path[0]])) ? null : parent[path[0]]);
	else
		return jsonGetVal(parent[path[0]], path.slice(1));
}
function jsonGetValDeep(parent, path) {
	if (path.length == 1)
		return (isUndef(parent) || isNull(parent)) ? null : ((isUndef(parent[path[0]]) || isNull(parent[path[0]])) ? null : jsonCpy(parent[path[0]]));
	else
